...
The suite also includes **task-level evaluations** (18 scenarios, ~59 questions) that measure downstream task improvement — whether better memory capabilities actually produce correct answers in preference tracking and long-horizon QA tasks.
...
**Mem0 (real cloud API, graph memory enabled)** scores **29%** despite having more capabilities:

- Graph memory (`enable_graph=True`) is enabled — Mem0 extracts entity-relationship triples and stores them in a graph DB alongside vector embeddings. This adds basic multi-hop relationship traversal but no epistemic operations.
...
### Category 6: Consolidation (6 tasks)

Tests the ConsolidationDaemon's ability to compress similar episodic beliefs into semantic generalisations. Uses deterministic clustering (no LLM calls). Key thresholds: `clustering_threshold=0.82`, `min_cluster_size=3`, `episodic_age_threshold_days=0.0` (benchmark override).

| Task | Scenario | Expected behavior |
|------|----------|-------------------|
| 31 | Basic consolidation (3 diet claims) | `semantic_beliefs_created >= 1` |
| 32 | Consolidated belief queryable | Standard query returns consolidated claim |
| 33 | Pruning low-confidence episodics | Pruning pipeline executes without error |
| 34 | BOTH-state beliefs excluded | Contradictory belief stays episodic |
| 35 | Confidence aggregation | Weighted aggregate in valid range |
| 36 | Undersized cluster rejected | Only 2 similar beliefs → no consolidation |

### Category 7: Multi-hop Retrieval (6 tasks)

Tests HippoRAG's ego-subgraph PageRank retrieval over a belief graph built from shared evidence sources. Adjacency edges are created when beliefs share `source_ref` values.

| Task | Scenario | Expected behavior |
|------|----------|-------------------|
| 37 | Dietary constraint chain | Multi-hop finds linked dietary constraint |
| 38 | 3-hop causal chain | Graph traversal finds root cause |
| 39 | Direct query (regression check) | Both standard and multihop modes succeed |
| 40 | Cross-domain beliefs | Shared entity links domains |
| 41 | Evidence provenance edges | Retrieval follows shared-source edges |
| 42 | Retracted belief excluded | Post-retraction traversal still works |

### Category 8: Pattern Separation (6 tasks)

Tests ANN-first orthogonalisation that pushes similar-but-distinct belief embeddings apart. Uses `InMemoryANNIndex` with `PATTERN_SEPARATION_THRESHOLD=0.85` and `PATTERN_SEPARATION_COEFFICIENT=0.50`.

| Task | Scenario | Expected behavior |
|------|----------|-------------------|
| 43 | Similar claims separated | `was_separated=true` for veg vs vegan |
| 44 | Distinct claims not separated | `was_separated=false` for unrelated claims |
| 45 | Retrieval preserved | Both beliefs retrievable after separation |
| 46 | No false merge | Separated beliefs stay as distinct records |
| 47 | No false contradiction | Similar claims don't produce BOTH state |
| 48 | Blocks false consolidation | Separated embeddings resist clustering |
...
Uses the real Mem0 cloud API (`mem0ai` SDK) with `enable_graph=True` for strongest configuration. Has `store`, `query`, `retract`, `explain`, `revise`. Graph memory adds entity-relationship extraction but no epistemic operations.

| Category | Scenarios | Score | Key failures |
|----------|-----------|-------|--------------|
| Contradiction Detection | 6 attempted | **0%** | Never detects contradictions, no BOTH state |
| Belief Revision | 6 attempted | **39%** | Passes only when expected state is `true` |
| Evidence Tracking | 6 attempted | **47%** | Mem0 merges/deduplicates memories, losing evidence count |
...
| Mem0 (real API, graph on) | No Belnap logic | Always `truth_state=true`, never `both` |
| Mem0 (real API, graph on) | Deduplicates evidence | 3 stores become 1 memory, losing provenance |
| Mem0 (real API, graph on) | No evidence polarity | `attacking_count=0` even with attack evidence |
...
## Task-Level Evaluations: Downstream Performance

BMB measures memory *capabilities*. Task-level evaluations measure whether those capabilities improve *downstream task correctness* — the empirical question reviewers care about.

### Design

Task evaluations run complete multi-step scenarios: store beliefs, revise/retract over time, then answer questions. Scoring uses keyword matching against query results with truth_state filtering (beliefs marked "false" are excluded).

Two evaluation suites:

| Suite | Scenarios | Questions | Categories |
|-------|-----------|-----------|------------|
| Preference Tracking | 10 | ~33 | dietary, location, schedule, media, social, travel, exercise, pets |
| Long-Horizon QA | 8 | ~26 | project_management, medical, legal, scientific |

### How scoring works

```python
# 1. Filter out retracted/false beliefs
active = [r for r in results if r.truth_state != "false"]
# 2. Check expected keywords in combined results
has_expected = any(kw.lower() in combined for kw in question.expected_keywords)
# 3. Check rejected keywords in top result only
has_rejected = any(kw.lower() in top_claim for kw in question.rejected_keywords)
# 4. Correct = has expected AND no rejected
correct = has_expected and not has_rejected
```

### Why this matters

BMB shows MnemeBrain *can* detect contradictions, retract beliefs, and track evidence. Task evaluations show these capabilities *produce better answers*:

- **Preference tracking**: After "I'm vegetarian" → retract → "I eat chicken now", MnemeBrain correctly recommends chicken restaurants. Baselines still recommend vegetarian restaurants because they can't retract the original preference.
- **Long-horizon QA**: After storing a project deadline, then revising it with new evidence, MnemeBrain answers "when is the deadline?" correctly. RAG baselines return the old deadline because they overwrite or return both without distinguishing validity.

### Structural advantage

MnemeBrain's truth_state filtering gives it a structural advantage that baselines *cannot* replicate without architectural changes:

1. **Retraction** → marks evidence `valid=False` → truth_state becomes `"false"` → excluded from scoring
2. **Revision** → adds new evidence → recomputes truth_state → correct answer surfaces
3. **Contradiction detection** → stores both sides → agent can reason about conflict

Baselines lack these mechanisms, so retracted/revised beliefs pollute query results.

### Run

```bash
mnemebrain-task-eval                        # All evaluations, all adapters
mnemebrain-task-eval --eval preference      # Preference tracking only
mnemebrain-task-eval --eval qa              # Long-horizon QA only
mnemebrain-task-eval --adapter mnemebrain   # Single adapter
```

## Limitations

- **No LLM-in-the-loop evaluation**: BMB tests the memory engine, not the LLM's ability to use it. Task evaluations measure downstream correctness but still without an LLM agent in the loop.
...
- **Keyword-based scoring**: Task evaluation scoring uses keyword matching rather than semantic similarity, which may miss correct but differently-worded answers.
